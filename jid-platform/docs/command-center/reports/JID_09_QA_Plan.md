...
| Check | Result |
|---|---|
| Specs 02â€“08 ledger status | SHIPPED (verified from tip ledger object) |
...
| `individual`, `anon` | Â§14 negative matrix |
| `assignApplicantA/B` | Staff assignment / view-only cells |

## Journeys (Spec Â§7) â€” execution ownership

### Journey 1 â€” Business (Session 09-B)

Signup â†’ verify â†’ reject â†’ reapply â†’ approve â†’ create draft â†’ edit/save/reload â†’ publish â†’ public page â†’ unpublish â†’ dashboard.

Locales: AR + EN. Primary flow also at 375px. Actors: business applicant/owner fixtures for this RUN_ID.

### Journey 2 â€” University (Session 09-B)

Same chain including reapply + snapshot-present and snapshot-absent dashboard honesty.

### Journey 3 â€” Staff (Session 09-B)

Queue â†’ filters â†’ auto-assign â†’ view-only (other reviewer) â†’ self-review block â†’ approve with no-auto-Profile notice â†’ reject with reason â†’ terminal read-only â†’ Super Admin explicit override.

### Journey 4 â€” Directory correction (Session 09-C)

Suggest â†’ staff review â†’ apply â†’ audit row verified in DB â†’ Directory updated. Use `correction-pending` fixture as starting point or recreate under the same RUN_ID if consumed.

### Journey 5 â€” Notifications (Session 09-C)
...
### Journey 6 â€” Publication (Session 09-C)

Full Spec 07 Â§22 walk for Business and University using draft/published/suspended fixtures.

## Negative authorization matrix (Spec Â§14) â€” Session 09-C

Every cell must be evidenced (screenshot and/or server denial note):

| Cell | Actor | Expectation |
|---|---|---|
| Anon â†’ staff routes | anon | redirect/404 |
| Anon â†’ owner routes | anon | denied |
| Anon â†’ draft/suspended public Profile | anon | 404 |
| Individual â†’ staff | individual | 404 |
| Staff B â†’ decide Staff A request | staffB on `vr-assigned-a` | denied UI + server |
| Admin invent override | _(no Admin actor; record absence)_ | no invented Admin override |
| Owner â†’ direct status manipulation | biz/uni owner | denied |
| Suggester â†’ apply own correction | correctionSuggester | denied |
| `/sys/claims` | any | must not exist |

## Accessibility passes (Spec Â§17) â€” Session 09-C
...
4. AR terminology sweep on rendered pages: no visible `Claim` / `Ù…Ø·Ø§Ù„Ø¨Ø©`; Latin digits in Arabic.

## Defect handling

- Journeys B/C **record** defects in `JID_09_Defect_Register.md` â€” they do not fix product code.
...
| 09-B | Journeys 1â€“3 evidence |
| 09-C | Journeys 4â€“6 + negative matrix + a11y + defect register |

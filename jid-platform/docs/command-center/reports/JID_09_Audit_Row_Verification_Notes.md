# JID Spec 09 -- Audit Row Verification Notes (Session 09-C)
...
## Journey 4 -- Directory correction

| Check | Result |
|---|---|
| Pending suggestion `correction-pending` present | YES -- status `pending`, field `city` |
| Directory target `dir-correction-target` present | YES -- slug `jid09-dir-corr-jid09-20260801-7d956c` |
| Directory `city` before Staff apply | `null` (unchanged; no apply completed in UI) |
| Staff approve/apply via UI | NOT COMPLETED -- Staff suggestions page failed to load (DEF-09B-002 / DEF-09C-002) |
...
| UI inbox (approval) | PASS -- Arabic copy states deliberate Profile creation; no auto-Profile implication |
| Live Staff decision -> new notification | NOT COMPLETED -- Staff verification workspace blocked by DEF-09B-002 |
...
## Journey 6 -- Publication audits

| Check | Result |
|---|---|
| Owner publish/unpublish UI | NOT OPERABLE -- owners redirected to `/signup/entity-type` under DEF-09B-002 |

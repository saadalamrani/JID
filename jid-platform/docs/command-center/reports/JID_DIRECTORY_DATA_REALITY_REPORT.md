# JID Directory Data Reality Report

**Generated:** 2026-08-23T02:27:42.688Z  
**Branch:** cursor/jid-directory-data-reality-v2  
**Base:** cursor/jid-interview-final-integration-v1 @ 7d78fbb

## Source resolution

| File | Status |
| --- | --- |
| JID_Catalog_Import_Manifest_2026-08-05.csv | Present (1000 rows) |
| JID_Catalog_Automated_Ingestion_and_Directory_Maintenance_Spec_v1.3.md | Copied to docs/command-center/specifications/ |

## Manifest dry-run totals

- Total: 1000
- Reconcile existing: 2
- Repeated-domain rows: 14
- High-confidence candidates: 106
- Low-confidence candidates: 863
- Sector review (incl. TAX_003 holding): 15

## Taxonomy

- Canonical regions in seed: 13
- Source regions represented: 11
- Absent from source: Hail, Al Bahah
- Canonical sectors in seed: 45
- Source main sectors: 36

## Safety

- Production: untouched
- Remote nonprod bulk import: **controlled founder manifest ingest completed 2026-08-23** (`founder.manifest-2026-08-05`, 1000 candidates, zero publication)
- Profiles / Verification / ownership: zero side effects

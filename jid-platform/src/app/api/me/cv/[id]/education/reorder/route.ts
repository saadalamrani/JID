...
import { cvEducationReorderSchema } from '@/lib/cv/schemas/education'
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const parsed = cvEducationReorderSchema.safeParse(body)
  if (!parsed.success) {
  ...
  const existingIds = new Set(cv.education.map((entry) => entry.id))
  if (
    parsed.data.orderedIds.length !== cv.education.length ||

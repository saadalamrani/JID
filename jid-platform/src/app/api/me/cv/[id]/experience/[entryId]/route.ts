...
import { cvExperienceDbUpdateSchema } from '@/lib/cv/schemas/experience'
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (!cv || !cv.experience.some((entry) => entry.id === entryId)) {
  ...
  const parsed = cvExperienceDbUpdateSchema.safeParse(body)
  if (!parsed.success) {
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  const remaining = cv.experience.filter((entry) => entry.id !== entryId)

...
export async function PublicFooter() {
  ...
  return (
    <footer className="border-t border-jid-line bg-white">
      <div className="container-jid py-10">
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {groups.map((group) => (
            <div key={group.title}>
              <h2 className="text-sm font-semibold text-jid-olive">{group.title}</h2>
              <ul className="mt-3 space-y-2">
                {group.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-jid-ink/70 transition-colors hover:text-jid-olive"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="mt-8 border-t border-jid-line/60 pt-6 text-center text-xs text-jid-ink/60">

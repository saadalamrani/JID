...
export async function PdplTrustBar() {
  ...
  return (
    <section className="border-b border-jid-line/60 bg-jid-olive py-8 text-white">
        ...
        <div className="flex flex-col items-start gap-6 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-4">
            <span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-white/10">
              <ShieldCheck className="h-6 w-6 text-jid-gold" aria-hidden />
            </span>
            <div>
              <h2 className="font-arabic text-lg font-semibold">{t('title')}</h2>
              <p className="mt-1 max-w-2xl text-sm leading-relaxed text-white/85">{t('body')}</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Link
              href="/pdpl"
              className="inline-flex items-center justify-center rounded-lg bg-jid-gold px-4 py-2 text-sm font-semibold text-jid-ink transition-colors hover:bg-jid-gold/90"
            >
              {t('cta')}
            </Link>
          </div>

...
export function SectorMultiSelect() {
  const { filters, sectors, toggleSector } = useJobFilters()
  const [open, setOpen] = useState(false)
  const selectedCount = filters.sectors.length

  return (
    <div className="space-y-2">
      <p className="font-arabic text-xs font-medium text-foreground-400">القطاع</p>
      <DropdownMenu open={open} onOpenChange={setOpen}>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            variant="outline"
            aria-label={
              selectedCount > 0 ? `القطاعات، ${selectedCount} محددة` : 'اختر القطاعات'
            }

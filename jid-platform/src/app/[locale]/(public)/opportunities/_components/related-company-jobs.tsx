...
export function RelatedCompanyJobs({ jobs }: RelatedCompanyJobsProps) {
  ...
  return (
    <section className="mt-10 border-t border-jid-line/30 pt-8" aria-label="فرص أخرى من نفس الجهة">
      <h2 className="font-arabic text-lg font-semibold text-jid-ink">فرص أخرى من نفس الجهة</h2>
        ...
        {jobs.map((job) => {
          ...
          return (
            <li key={job.id}>
              <LocaleLink
                href={href}
                className="block rounded-xl border border-jid-line/40 bg-white p-4 shadow-sm transition-shadow hover:shadow-md"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h3 className="font-arabic text-base font-semibold text-jid-ink">{title}</h3>
                    <p className="mt-1 font-arabic text-xs text-jid-ink/60">{experienceLabel}</p>
                  </div>
                  <DeadlineBar

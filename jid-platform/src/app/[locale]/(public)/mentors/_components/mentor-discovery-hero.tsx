...
export function MentorDiscoveryHero() {
  ...
  return (
    <header className="space-y-3 pb-6">
      <h1 className="font-arabic text-3xl font-semibold text-jid-ink">{t('title')}</h1>
      <p className="font-arabic text-sm text-jid-ink/60">{t('subtitle')}</p>
      <dl className="flex flex-wrap gap-4 font-arabic text-sm">
        <div className="rounded-lg border border-jid-line bg-white px-4 py-3 shadow-sm">
          <dt className="text-jid-ink/50">{t('activeMentors')}</dt>
          <dd className="mt-1 text-xl font-semibold text-jid-olive">
            {isFetching ? (
        ...
        <div className="rounded-lg border border-jid-line bg-white px-4 py-3 shadow-sm">
          <dt className="text-jid-ink/50">{t('totalSessions')}</dt>
          <dd className="mt-1 text-xl font-semibold text-jid-olive">
            {isFetching ? (
              <span className="inline-block h-6 w-12 animate-pulse rounded bg-jid-line/30" />
            ) : (

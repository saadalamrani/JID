...
export function FeaturedMentorsSection({ mentors }: FeaturedMentorsSectionProps) {
  ...
  return (
    <section className="container-jid border-t border-jid-line/40 py-16">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 className="font-arabic text-2xl font-semibold text-jid-ink">{t('title')}</h2>
          <p className="mt-1 font-arabic text-sm text-jid-ink/60">{t('subtitle')}</p>
        </div>
        <Link href="/mentors" className="font-arabic text-sm text-jid-olive hover:underline">
          {t('browseAll')}
        </Link>
      </div>

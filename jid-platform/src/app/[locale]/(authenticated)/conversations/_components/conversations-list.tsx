  ...
  if (conversations.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-white px-6 py-16 text-center">
        <p className="font-arabic text-sm font-medium text-foreground">{t('emptyTitle')}</p>
        <p className="mt-2 font-arabic text-sm text-foreground/55">{t('emptyBody')}</p>
      </div>
    )
  }
          ...
          <li key={conversation.id}>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate font-arabic text-sm font-medium text-foreground">{name}</p>

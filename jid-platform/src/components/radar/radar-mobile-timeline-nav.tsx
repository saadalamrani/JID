...
export function RadarMobileTimelineNav({ userId, meetings }: RadarMobileTimelineNavProps) {
  ...
  return (
    <>
      <nav
        className="fixed inset-x-0 bottom-0 z-40 border-t border-jid-line/60 bg-white/95 backdrop-blur-sm lg:hidden"
        ...
        <div className="container-jid flex justify-center py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
          <button
            ...
            className={cn(
              ...
              open ? 'bg-jid-olive text-white' : 'bg-jid-beige/60 text-jid-ink hover:bg-jid-beige',
            )}
            onClick={() => {
              setOpen(true)
          ...
          >
            ...
            <span className="rounded-full bg-white/90 px-2 py-0.5 text-xs font-semibold text-jid-olive">
              {meetings.length}
            </span>
          </button>
        </div>

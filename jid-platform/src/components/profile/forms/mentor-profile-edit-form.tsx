...
export function MentorProfileEditForm({ mentor }: MentorProfileEditFormProps) {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="container-jid space-y-6 py-8">
      <h1 className="text-xl font-semibold text-jid-ink">{t('mentorTitle')}</h1>
      ...
      <section className="rounded-xl border border-jid-line bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-sm font-medium text-jid-ink/70">{t('sectionCareer')}</h2>
          <Button
            type="button"
            size="sm"
            variant="outline"
        ...
        <div className="space-y-4">
          {career.fields.map((field, index) => (
            <div key={field.id} className="space-y-3 rounded-lg border border-jid-line p-4">
      ...
      <section className="rounded-xl border border-jid-line bg-white p-5 shadow-sm">
        <h2 className="mb-3 text-sm font-medium text-jid-ink/70">{t('sectionExpertise')}</h2>
        <div className="flex flex-wrap gap-2">
          {sectors.map((sector, index) => (
            <span key={`${sector}-${index}`} className="inline-flex items-center gap-1 rounded-full bg-jid-olive/10 px-3 py-1 text-xs text-jid-olive">
              {sector}
              <button
                type="button"
                onClick={() =>
      ...
      <div className="flex gap-3">
        <Button type="submit" className={cn('bg-jid-olive hover:bg-jid-olive/90')} disabled={form.formState.isSubmitting}>
          {t('save')}
        </Button>

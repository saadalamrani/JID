...
export function MentorExpertiseSection({ sectors, yearsExperience }: MentorExpertiseSectionProps) {
  ...
  return (
    <section className="rounded-xl border border-jid-line bg-white p-5 shadow-sm">
      <h2 className="mb-4 text-sm font-medium text-jid-ink/70">{t('expertiseTitle')}</h2>

      <dl className="space-y-4">
        {yearsExperience != null ? (
          <div>
            <dt className="text-xs text-jid-ink/50">{t('yearsExperienceLabel')}</dt>
            <dd className="mt-1 text-sm font-medium text-jid-ink">
        ...
        <div>
          <dt className="text-xs text-jid-ink/50">{t('sectorsLabel')}</dt>
          <dd className="mt-2">
            {sectors.length > 0 ? (
              ...
              <p className="text-sm text-jid-ink/50">{t('sectorsEmpty')}</p>
            )}
          </dd>
        </div>
      </dl>

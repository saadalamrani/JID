...
import {
  REJECTION_EMAIL_SUBJECT_AR,
  renderRejectionEmailHtml,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (application.status !== 'rejected') {
      return jsonResponse({ error: 'Application is not rejected' }, 400)
    }
    ...
    await sendResendEmail({
      to,
      subject: REJECTION_EMAIL_SUBJECT_AR,
      html: renderRejectionEmailHtml(vars),
    })

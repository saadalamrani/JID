...
CREATE OR REPLACE FUNCTION public.can_read_individual_profile(p_profile_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.can_read_individual_profile(uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.can_read_individual_profile(uuid)
  TO anon, authenticated, service_role;

COMMENT ON FUNCTION public.can_read_individual_profile(uuid) IS
...
CREATE POLICY profile_skills_audience_read
  ...
  USING (
    public.can_read_individual_profile(profile_id)
  );
...
FROM public.profiles p
WHERE public.can_read_individual_profile(p.id);
...
WHERE public.can_read_individual_profile(ps.profile_id);
...
JOIN public.universities_catalog uc ON uc.id = uds.university_id
WHERE EXISTS (
  SELECT 1
  FROM public.university_profiles up
  JOIN public.companies c ON c.id = up.directory_id
...
COMMENT ON MATERIALIZED VIEW public.university_dashboard_snapshot IS
  'Consented active-profile aggregates with independent CV, application, and completed-meeting counts.';
COMMENT ON VIEW public.university_dashboard_view IS
  'Owner-scoped aggregate university dashboard; never exposes individual rows.';

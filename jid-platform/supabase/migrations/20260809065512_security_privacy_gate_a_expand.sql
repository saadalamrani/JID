-- JID Security & Privacy Contract Remediation — Gate A EXPAND
-- Additive, backward-compatible with the currently deployed e876060/fc852e5 app.
-- Creates private audience helpers, safe public projections, tightened mentor-review
-- insert binding, and consent-safe University aggregates/fail-closed owner view.
-- Does NOT remove base-table public policies or anon/authenticated grants the
-- currently deployed application still requires. Those removals live in CONTRACT.
...
-- Additive audience policy for profile_skills. Legacy profile_skills_public_read
-- remains until CONTRACT so the currently deployed application keeps working.
...
-- ---------------------------------------------------------------------------
-- Mentor public projections and review integrity (insert binding only).
-- Base-table public select policies/grants remain until CONTRACT.
-- ---------------------------------------------------------------------------
...
-- Owner view fails closed until Directory-to-catalog identity is reconciled.
-- ---------------------------------------------------------------------------

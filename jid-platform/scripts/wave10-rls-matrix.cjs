/**
 * Wave 10 RLS/privacy actor matrix against jid-nonprod.
...
async function main() {
  ...
  try {
    const body = fs.readFileSync(
      path.join(root, 'tests/rls/wave10-university-foundation.matrix.sql'),
      'utf8',
    )
    const result = await sql.unsafe(body)
    const tokens = JSON.stringify(result)
    if (!tokens.includes('WAVE10_RLS_ACTOR_MATRIX_PASS')) {
      throw new Error('unexpected matrix result: ' + tokens.slice(0, 500))
    }
    process.stdout.write('WAVE10_RLS_MATRIX PASS\n')
  } finally {

/**
 * Section 5 — /staff layout guard verification.
 * Run with dev server: pnpm dev
 * Then: pnpm tsx scripts/verify-staff-layout-guards.ts
 */

const BASE = process.env.VERIFY_BASE_URL ?? 'http://localhost:3000'
const TARGET = '/staff/dashboard'
...
const cases: Case[] = [
  ...
  {
    label: 'staff without MFA → redirect /staff/mfa',
    headers: { 'x-jid-test-role': 'staff' },
    expectStatus: 307,
    expectLocationIncludes: '/staff/mfa',
  },
  {
    label: 'staff expired session (8h+) → redirect /staff/login?reason=expired',
    headers: {
      'x-jid-test-role': 'staff',
      'x-jid-test-aal2': 'true',
      'x-jid-test-session-issued-at': String(Math.floor(Date.now() / 1000) - 28801),
    },
    expectStatus: 307,
    expectLocationIncludes: '/staff/login',
  },
  {
    label: 'staff fresh session + MFA → 200 shell',
    headers: {
      'x-jid-test-role': 'staff',
      'x-jid-test-aal2': 'true',
    },
...
async function main() {
  console.log(`Section 5 — /staff guard verification (${BASE}${TARGET})\n`)
